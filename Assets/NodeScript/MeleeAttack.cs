...
public class MeleeAttack : ActionNode
{
    ...
    private void CheckAttackType()
    {
        if( Time.time - startTime > delayAttack)
        {
            ...
            if (isEnemyAttackPlayerDirection) AttackPlayerDirection();
            else if (isEnemyAttackShortestAxis) AttackShortestAxis();
            else if (isEnemyAttackOnlyXAxis) AttackOnlyXAxis();
            else if (isEnemyAttackOnlyYAxis) AttackOnlyYAxis();
            else if (isEnemyAttackBothAxis) AttackBothAxis();
            else if (isEnemyAttack4Direction) Attack4Direction();
            else if (isEnemyAttack4Angle) Attack4DirectionAngle();
        }
    }
